        ...
        try {
           ...
           paramv.setTransaction_id(request.getReferenceCode().replace("/", "^").replace("_", "."));
            ...
            if (resultv.indexOf("T24.ID") > 0) {
                ...
                ofsoptions = new String[]{"", "I", "VALIDATE", "2", "0"};
                param.setOptions(ofsoptions);
...
//                items.add(item);
                ...
                String amt = result.split(",")[0].split("=")[1];

                result = output.substring(output.indexOf("ACCOUNT.NUMBER:1:1="));

                String acct = result.split(",")[0].split("=")[1];

                Double Amount = Double.parseDouble(amt);

                Double RequestAmt = Double.parseDouble(request.getAmount());

                if (RequestAmt <= Amount) {

                    if (RequestAmt < Amount) {

                        Double newamt = Amount - RequestAmt;
                        ...
                        item = new DataItem();
                        item.setItemHeader("ACCOUNT.NUMBER");
                        item.setItemValues(new String[]{acct});
                        items.add(item);

                        item = new DataItem();
                        item.setItemHeader("LOCKED.AMOUNT");
                        item.setItemValues(new String[]{newamt.toString()});
                        items.add(item);
...
//                  items.add(item);
                        ...
                        ofsoptions = new String[]{"", "R", "PROCESS", "2", "0"};
